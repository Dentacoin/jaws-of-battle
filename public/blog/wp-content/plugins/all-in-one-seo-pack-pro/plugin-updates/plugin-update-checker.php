...
class PluginUpdateChecker {
	...
	public $checkPeriod;      //How often to check for updates (in hours).
	...
	function installHooks(){
		...
		if ( $this->checkPeriod > 0 ){
			...
			add_action($cronHook, array(&$this, 'maybeCheckForUpdates'));
	...
	function injectInfo($result, $action = null, $args = null){
		...
		if ($pluginInfo) {
			if ( method_exists( $pluginInfo, 'toWpFormat' ) )
	...
	function injectUpdate($updates){
		...
		if ( !empty($state) && isset($state->update) && !empty($state->update) ){
			...
			if ( version_compare($state->update->version, $this->getInstalledVersion(), '>') ) {
				if ( method_exists( $state->update, 'toWpFormat' ) )
