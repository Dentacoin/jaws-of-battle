...
if ( !function_exists( 'aioseop_embed_handler_html' ) ) {
...
if ( !function_exists( 'aioseop_ajax_update_oembed' ) ) {
	function aioseop_ajax_update_oembed() {
		...
		if ( !empty( $ids ) ) {
			$ids = explode( ',', $ids );
			$ids = get_posts( Array( 'numberposts' => $ids[1] - $ids[0], 'offset' => $ids[0], 'post_type' => $post_types, 'post_status' => 'publish', 'fields' => 'ids' ) );
